...
public class OpenRoomDialog extends DialogFragment {
    ...
    public OpenRoomDialog(Context context, OnOpenRoomListen listen) {
        mContext = context;
        mListen = listen;
    }
    public interface OnOpenRoomListen {
        void onDataBack(String continued,String clientName);
    }
    @Override
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        final EditText clientName = (EditText) view.findViewById(R.id.dialog_client_name);
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {//设置节目名称
                if (TextUtils.isEmpty(clientName.getText().toString())||TextUtils.isEmpty(continued.getText().toString())) {
                    ToastUtil.showToast(mContext, getString(R.string.open_room_title), 3000);
                } else {
                    if (mListen != null){
                        mListen.onDataBack(continued.getText().toString(),clientName.getText().toString());
                    }
                    dialog.dismiss();
                }
            }
